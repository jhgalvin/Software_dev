...
Class profileCreateClass
{
	...
	public function __construct(Array $post)
	{
		...
		$this->companyState=$this-=>['companyState'];
		$this->companyZipCode = $this->post['companyZipCode'];
		
		

		$sql = "SELECT * FROM user WHERE company_ID = '$this->company_ID'";
	...
	public function profileFill(){
		...
		try
		{
			...
			echo '<script type="text/JavaScript">
                	alert("An error occured when updating profile.");
               		window.location.replace("http://www.zoonika.com/profileUpdateForm.php");
                	</script>'
			;
		}
